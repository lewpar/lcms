...
const { readSites, writeSites, siteDir, settingsFile, ensureDirs, slugify, isReservedSlug, OUTPUT_DIR, DOCS_DIR } = require('../lib/paths');
...
// Migration: ensure all sites have a non-empty name
(function migrateSites() {
  const sites = readSites();
  const fixed = sites.map(s => ({ ...s, name: s.name || 'Untitled' }));
  if (fixed.some((s, i) => s.name !== sites[i].name)) writeSites(fixed);
})();
...
router.post('/', (req, res) => {
  const name = (req.body.name || '').trim().slice(0, MAX_STR.name);
  if (!name) return res.status(400).json({ error: 'Site name is required.' });
  const rawSlug = req.body.slug ? String(req.body.slug).trim().slice(0, MAX_STR.slug) : slugify(name);
  ...
  if (sites.some(s => s.slug === rawSlug)) {
    return res.status(409).json({ error: `A site with slug "${rawSlug}" already exists.` });
  }
  const id = uuidv4();
  ensureDirs(id);
  try {
...
router.patch('/:siteId', requireValidSiteId, (req, res) => {
  ...
  if (req.body.name !== undefined) {
    ...
    if (!newName) return res.status(400).json({ error: 'Site name is required.' });
  ...
  if (req.body.slug !== undefined) {
    const newSlug = String(req.body.slug).trim().slice(0, MAX_STR.slug);
    const slugError = validateSlug(newSlug);
    if (slugError) return res.status(400).json({ error: slugError });
    const slugDuplicate = sites.find(s => s.id !== req.params.siteId && s.slug === newSlug);
    if (slugDuplicate) return res.status(409).json({ error: `A site with slug "${newSlug}" already exists.` });
