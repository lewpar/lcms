...
body.no-sidebar .content-area{margin-left:0}
body.no-sidebar .hamburger{display:none!important}
body.no-sidebar .top-bar{display:none}
...
.floating-dark-mode-btn{position:fixed;top:20px;right:20px;z-index:200;width:38px;height:38px;padding:0;border-radius:50%;font-size:16px;background:var(--surface);border:1px solid var(--border);color:var(--text);box-shadow:0 2px 8px rgba(0,0,0,.18)}.floating-dark-mode-btn:hover{background:var(--surface-hover,var(--border))}
...
const DARK_MODE_JS = `(function(){
  ...
  document.addEventListener('DOMContentLoaded',function(){
    ...
    btns.forEach(function(btn){btn.textContent=isDark?'☀':'☽';});
    btns.forEach(function(btn){
      btn.addEventListener('click',function(){
        var current=document.documentElement.getAttribute('data-theme');
        var next=current==='dark'?'light':'dark';
        document.documentElement.setAttribute('data-theme',next);
        document.querySelectorAll('.dark-mode-btn').forEach(function(b){b.textContent=next==='dark'?'☀':'☽';});
...
<nav class="page-prev-next">
  ...
  const disableNav = !!settings.disableNav;
  const floatingDarkMode = !!settings.floatingDarkMode;
...
<body${disableNav ? ' class="no-sidebar"' : ''}>
${showDarkMode ? `<script>${DARK_MODE_JS}</script>` : ''}
<script>${COPY_LINK_JS}</script>
${floatingDarkMode ? `<button class="dark-mode-btn floating-dark-mode-btn" title="Toggle dark mode" aria-label="Toggle dark mode">☽</button>` : ''}
<div class="app-layout">
  ${disableNav ? '' : sidebarHtml(settings, navItems, slug, toc)}
  ${disableNav ? '' : '<div class="sidebar-overlay" id="overlay"></div>'}
...
${floatingDarkMode ? `<button class="dark-mode-btn floating-dark-mode-btn" title="Toggle dark mode" aria-label="Toggle dark mode">☽</button>` : ''}
<div class="app-layout">
  ${disableNav ? '' : sidebarHtml(settings, navItems, null)}
  ${disableNav ? '' : '<div class="sidebar-overlay" id="overlay"></div>'}
  <div class="content-area">
    ${customHeaderIdx}
    <header class="top-bar">
